...
const UserDashboard = ({ user }) => {
    ...
    const [view, setView] = React.useState('sod'); 
    const [reportData, setReportData] = React.useState({ sod: [], eod: [] }); 
    ...
    const generateReport = (type) => {
        ...
        if (type === 'SOD') { text = `🚀 SOD Report - ${headerDate}\n\nTasks for Tomorrow/Next Shift:\n`; if (!reportData.sod || reportData.sod.length === 0) text += "No tasks assigned yet."; else reportData.sod.forEach(item => { text += `• ${item.start} - ${item.end}: ${item.task}\n`; }); } 
    ...
    return (
        <div className="pb-24">
            ...
            <div className="p-4 max-w-lg mx-auto">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl mb-6"><button onClick={() => setView('sod')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${view === 'sod' ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500'}`}>SOD (Plan)</button><button onClick={() => setView('eod')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${view === 'eod' ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500'}`}>EOD (Done)</button></div>
                <div className="space-y-6 fade-in">
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                        <h3 className="font-bold text-slate-700 mb-3 flex items-center gap-2"><Plus size={18} className="text-brand-500"/> {view === 'sod' ? 'Plan for Tomorrow' : 'Log Today\'s Activity'}</h3>
                        {view === 'sod' ? ( <div className="grid grid-cols-2 gap-2 mb-3"><datalist id="time-options">{TIME_OPTIONS.map(t => <option key={t} value={t} />)}</datalist><div><label className="text-[10px] uppercase font-bold text-slate-400">Start</label><input list="time-options" className="w-full p-2 bg-slate-50 rounded-lg border-transparent text-sm" value={newTask.start} onChange={(e) => setNewTask({...newTask, start: e.target.value})} /></div><div><label className="text-[10px] uppercase font-bold text-slate-400">End</label><input list="time-options" className="w-full p-2 bg-slate-50 rounded-lg border-transparent text-sm" value={newTask.end} onChange={(e) => setNewTask({...newTask, end: e.target.value})} /></div></div> ) : ( <div className="mb-3"><label className="text-[10px] uppercase font-bold text-slate-400">Duration (Minutes)</label><input type="number" value={newTask.minutes} onChange={(e) => setNewTask({...newTask, minutes: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg text-sm" placeholder="e.g. 45" /></div> )}
                    ...
                    filteredEmployees.map((emp) => {
                        ...
                        return (
                            <div key={emp.uid} className={`bg-white rounded-xl shadow-sm border overflow-hidden fade-in ${isSuspended ? 'border-red-200 bg-red-50/50' : 'border-slate-200'}`}>
                                ...
                                {hasActivity ? ( <div className="p-4 grid md:grid-cols-2 gap-4"><div className="bg-blue-50/30 p-3 rounded-lg border border-blue-50"><h4 className="text-xs font-bold text-brand-600 mb-2 uppercase tracking-wide">SOD Plan</h4>{(!report.sod || report.sod.length === 0) ? <p className="text-xs text-slate-400 italic">Not submitted</p> : ( <ul className="space-y-2">{report.sod.map((item, i) => (<li key={i} className="text-xs text-slate-700 flex gap-2"><span className="font-mono text-slate-400 whitespace-nowrap min-w-[60px]">{item.start}</span><span>{item.task}</span></li>))}</ul> )}</div><div className="bg-green-50/30 p-3 rounded-lg border border-green-50"><div className="flex justify-between items-center mb-2"><h4 className="text-xs font-bold text-green-600 uppercase tracking-wide">EOD Results</h4><span className="text-[10px] font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{formatHours(userTotalMins)}h Total</span></div>{(!report.eod || report.eod.length === 0) ? <p className="text-xs text-slate-400 italic">Not submitted</p> : ( <ul className="space-y-2">{report.eod.map((item, i) => (<li key={i} className="text-xs text-slate-700 flex justify-between gap-4"><span>{item.task}</span><span className="font-bold text-green-700 whitespace-nowrap">{formatHours(item.minutes)}h</span></li>))}</ul> )}</div></div> ) : ( <div className="p-6 text-center"><div className="inline-block p-3 bg-slate-50 rounded-full mb-2"><Calendar size={20} className="text-slate-300"/></div><p className="text-sm text-slate-400 font-medium">No Activities Yet</p><p className="text-xs text-slate-300">Employee has not submitted any reports for this date.</p></div> )}
                            </div>
                        )
                    })
                )}
